...
Formally, in Category Theory, a \textbf{Functor} is a mapping between two categories $C$ and $D$ that preserves the structure of the categories (objects and morphisms). As famously defined by Saunders Mac Lane in \textit{``Categories for the Working Mathematician''}, a Functor $F: C \to D$ consists of:
\begin{itemize}
    \item An object mapping: For every object $X$ in $C$, an object $F(X)$ in $D$.
    \item A morphism mapping: For every morphism $f: X \to Y$ in $C$, a morphism $F(f): F(X) \to F(Y)$ in $D$, such that identities and composition are preserved.
\end{itemize}
