...
\begin{itemize}
    \item \textbf{The Original Definition}: Starting with the original definition from Category Theory (referencing Saunders Mac Lane).
    ...
    \begin{itemize}
        ...
        \begin{enumerate}
            \item \textit{Non-Endofunctor}: Mapping from Sets to \texttt{Hask}.
            \item \textit{Non-parametric Functor}: Inspects types (forbidden in Haskell).
            \item \textit{Restricted Functor}: Applies only to a subset of types.
        \end{enumerate}
    \end{itemize}
...
\begin{itemize}
    \item \textbf{Foundational Blocks}: \texttt{Const r} ($c$) and \texttt{Identity} ($X$).
    \item \textbf{Combinations}: How Sums and Products build more complex types.
\end{itemize}

\section{Functors out of Proxy and Identity}
\begin{itemize}
    \item \textbf{The Sum (Maybe)}: $1 + X$.
    \item \textbf{The Product (Identity)}: $1 \times X = X$.
    \item \textbf{Extensions}: Writer Monad ($r \times X$).
\end{itemize}

\section{Functors entirely out of Proxy}
\begin{itemize}
    \item \textbf{Fundamental Rule}: Proxy as the number 1.
    \item \textbf{Proxy + Proxy}: \texttt{Const Bool} ($1 + 1 = 2$).
    \item \textbf{Proxy * Proxy}: \texttt{Proxy} ($1 \times 1 = 1$).
\end{itemize}

\section{Recursion and Fixed Points}
\begin{itemize}
    \item \textbf{Recursive Sums/Products}: Building the \texttt{List}.
    \item \textbf{Shape Equations}: $L(X) = 1 + X \times L(X)$.
    \item \textbf{Alternative Shapes}: Binary Trees.
\end{itemize}
