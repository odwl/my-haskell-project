...
\item
  ...
  \tightlist
  \item
    \hyperref[section-11-void-0-inhabitants--initial-object]{Section
    1.1: \texttt{Void} (0 Inhabitants / Initial Object)}
    ...
    \item
      ...
      \end{itemize}
    \item
      \hyperref[3-type-level-guarantees]{3. Type-Level Guarantees}
      ...
      \end{itemize}
    \item
      \hyperref[4-type-level-phantom-types-for-type-safety]{4.
      ...
      \end{itemize}
    \item
      \hyperref[5-category-theory-the-initial-object-in-hask]{5.
      Category Theory: The Initial Object in Hask}
    \end{itemize}
  \item
    \hyperref[section-12--1-inhabitant--terminal-object]{Section 1.2:
    \texttt{()} (1 Inhabitant / Terminal Object)}
    ...
    \item
      \hyperref[5-the-curious-case-of-void]{5. The Curious Case of
      \texttt{{[}Void{]}}}
    \item
      \hyperref[6-category-theory-the-terminal-object-in-hask]{6.
      Category Theory: The Terminal Object in Hask}
    \item
      \hyperref[7-exercises-the-power-of-one]{7. Exercises: The Power of
      One}
    ...
    \end{itemize}
  \item
    \hyperref[section-13-bool-2-inhabitants--coproduct-of-terminal-objects]{Section
    1.3: \texttt{Bool} (2 Inhabitants / Coproduct of Terminal Objects)}
    ...
    \item
      \hyperref[2-using-either--]{2. Using \texttt{Either\ ()\ ()}}
    \item
      \hyperref[3-custom-enumerations]{3. Custom Enumerations}
    \item
      \hyperref[4-category-theory-coproduct-of-terminal-objects]{4.
      Category Theory: Coproduct of Terminal Objects}
    \end{itemize}
  \item
    \hyperref[section-14-other-finite-inhabitants-products-and-coproducts]{Section
    1.4: Other Finite Inhabitants (Products and Coproducts)}

    \begin{itemize}
    \tightlist
    \item
      \hyperref[1-a-3-inhabitant-type]{1. A 3-Inhabitant Type}
    \item
      \hyperref[2-playing-with-4-inhabitants]{2. Playing with 4
      Inhabitants}
    \item
      \hyperref[3-scaling-to-5-inhabitants]{3. Scaling to 5 Inhabitants}
    \end{itemize}
  \item
    \hyperref[section-15-infinite-inhabitants-countable-and-uncountable]{Section
    1.5: Infinite Inhabitants (Countable and Uncountable)}

    \begin{itemize}
    \tightlist
    \item
      \hyperref[1-countably-infinite-aleph_0]{1. Countably Infinite
      (\(\aleph_0\))}
    \item
      \hyperref[2-uncountably-infinite-2aleph_0]{2. Uncountably Infinite
      (\(2^{\aleph_0}\))}

      \begin{itemize}
      \tightlist
      \item
        \hyperref[a-infinite-streams]{A. Infinite Streams}
      \item
        \hyperref[b-infinite-functions]{B. Infinite Functions}
      \end{itemize}
    \end{itemize}
  \end{itemize}
\item
  \hyperref[annex-the-category-hask-]{Annex: The Category Hask}

  \begin{itemize}
  \tightlist
  \item
    \hyperref[hask-the-category-of-haskell-types]{Hask: The Category of
    Haskell Types}
  \item
    ...
    \tightlist
    \item
      \hyperref[interacting-with-bottom-safely-using-io]{Interacting
      with Bottom safely using \texttt{IO}}
    \end{itemize}
  \end{itemize}
\end{itemize}
...
This document explores both halves of that dichotomy: \textbf{The
Structures} and \textbf{The Algebras}. We will see how classifying types
by their inhabitants provides the foundation, and how layering
mathematical laws over those shapes creates the powerful abstractions we
...
provides the foundation. We will first master the \emph{Structures}, and
then dive deep into the \emph{Algebras} that bring them to life.
...
\texttt{haskell\ \ \ \ \textbackslash{}case\ \{\}} 2. \textbf{Using
standard library tools (\texttt{absurd})}: If you use the standard
library's \texttt{Void} type instead of a custom \texttt{Never}, the
library provides a function called
\texttt{absurd\ ::\ Void\ -\textgreater{}\ a}. Because \texttt{absurd}
can return \emph{any} type \texttt{a}, you can simply use it to return
\texttt{Void} right back!
\texttt{haskell\ \ \ \ f\ \_\ imposs\ \_\ =\ absurd\ imposs}
...
uninhabited types: Type-Level Guarantees and Type-Level Phantom Types
for Type Safety.

\paragraph{3. Type-Level Guarantees}\label{type-level-guarantees}
...
\paragraph{4. Type-Level Phantom Types for Type
...
\paragraph{5. Category Theory: The Initial Object in
Hask}\label{category-theory-the-initial-object-in-hask}

In the mathematical framework of Category Theory, Haskell types and
functions form a category colloquially known as \textbf{Hask}. In this
category, the types are the ``objects'' and the functions are the
``morphisms'' (arrows) between them.

Within \texttt{Hask}, \texttt{Void} plays the formal role of the
\textbf{Initial Object} (often denoted as \(0\)). An initial object
gives rise to a universal property: for \emph{every} other object \(A\)
in the category, there exists exactly \textbf{one unique morphism} from
the initial object to \(A\). In Haskell, this corresponds perfectly to
the function:

\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{absurd ::} \DataTypeTok{Void} \OtherTok{{-}\textgreater{}}\NormalTok{ a}
\end{Highlighting}
\end{Shaded}

There is exactly one conceptually pure way to implement this function
(by doing nothing, since it can never be called), making it the unique
morphism guaranteed by the initial object. Conversely, there are
\emph{no} general morphisms from populated types into \texttt{Void}.
...
\paragraph{\texorpdfstring{5. The Curious Case of
\texttt{{[}Void{]}}}{5. The Curious Case of {[}Void{]}}}\label{the-curious-case-of-void}

What happens if we create a list of \texttt{Void}? Does
\texttt{{[}Void{]}} exist?

Let's use Type Algebra. A list type \texttt{{[}a{]}} is defined
structurally as either an empty list \texttt{{[}{]}}, or a head element
\texttt{a} attached to a tail list \texttt{{[}a{]}}. Mathematically:
\texttt{List(a)\ =\ 1\ +\ a\ *\ List(a)} \emph{(Where \texttt{1}
represents the empty list \texttt{{[}{]}}, addition represents the
\texttt{Either} choice, and multiplication represents pairing head and
tail)}

If we substitute \texttt{Void} (which has 0 inhabitants):
\texttt{List(Void)\ =\ 1\ +\ 0\ *\ List(Void)}
\texttt{List(Void)\ =\ 1\ +\ 0} \texttt{List(Void)\ =\ 1}

The cardinality of \texttt{{[}Void{]}} is exactly \textbf{1}. Because
\texttt{Void} has no values, it is impossible to ever construct the
``head'' of a list of \texttt{Void}. The only value that can ever exist
of type \texttt{{[}Void{]}} is the empty list \texttt{{[}{]}}.
Therefore, \texttt{{[}Void{]}} is perfectly structurally isomorphic to
\texttt{()}!

\paragraph{6. Category Theory: The Terminal Object in
Hask}\label{category-theory-the-terminal-object-in-hask}

If \texttt{Void} is the Initial Object in the \texttt{Hask} category,
then the Unit type \texttt{()} is the \textbf{Terminal Object} (often
denoted as \(1\)).

The universal property of a terminal object dictates that for
\emph{every} other object \(A\) in the category, there exists exactly
\textbf{one unique morphism} from \(A\) to the terminal object. In
Haskell, this corresponds to a function that takes any type and returns
\texttt{()}:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{const}\OtherTok{ () ::}\NormalTok{ a }\OtherTok{{-}\textgreater{}}\NormalTok{ ()}
\end{Highlighting}
\end{Shaded}

There is exactly one conceptually pure way to implement this function
(by ignoring the input and returning the only available value of the
output type, which is precisely what the standard library's
\texttt{const\ ()} does). The fact that every type can be
deterministically mapped to \texttt{()} is what makes \texttt{()} the
terminal object in \texttt{Hask}.

\paragraph{7. Exercises: The Power of
...
\paragraph{4. Category Theory: Coproduct of Terminal
Objects}\label{category-theory-coproduct-of-terminal-objects}

In Category Theory, the sum of two objects is called a
\textbf{Coproduct}. As we saw, \texttt{Bool} is isomorphic to
\texttt{Either\ ()\ ()}. Because \texttt{()} is the terminal object
\(1\), \texttt{Bool} is the coproduct of \(1 + 1\). Its existence allows
us to represent independent binary choices in the \texttt{Hask}
category. Any type with exactly two inhabitants is conceptually
isomorphic to this same \(1+1\) coproduct.

\subsubsection{Section 1.4: Other Finite Inhabitants (Products and
Coproducts)}\label{section-1.4-other-finite-inhabitants-products-and-coproducts}

We have explicitly covered types with 0, 1, and 2 inhabitants. By
combining what we've learned, we can accurately determine the
cardinality of \emph{any} finite algebraic type through the mathematical
laws of addition (Coproducts, represented by \texttt{Either}) and
multiplication (Products, represented by tuples).

\paragraph{1. A 3-Inhabitant Type}\label{a-3-inhabitant-type}

The standard library provides \texttt{Ordering}, a type with exactly
three values representing the result of a comparison:
...
Like \texttt{Bool}, this is a sum of terminal objects:
\(1 + 1 + 1 = 3\).

\paragraph{2. Playing with 4
Inhabitants}\label{playing-with-4-inhabitants}

Because \(4 = 2 \times 2\), we can elegantly represent a 4-inhabitant
type using the Categorical Product (a tuple) of two \texttt{Bool}s!

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{type} \DataTypeTok{Four} \OtherTok{=}\NormalTok{ (}\DataTypeTok{Bool}\NormalTok{, }\DataTypeTok{Bool}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

This gives us exactly four values: \texttt{(True,\ True)},
\texttt{(True,\ False)}, \texttt{(False,\ True)}, and
\texttt{(False,\ False)}.

Alternatively, since \(4 = 1 + 3\), we could also represent it using a
Coproduct (sum type) of \texttt{()} and \texttt{Ordering}:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{type} \DataTypeTok{FourSum} \OtherTok{=} \DataTypeTok{Either}\NormalTok{ () }\DataTypeTok{Ordering}
\end{Highlighting}
\end{Shaded}

This also gives exactly four values: \texttt{Left\ ()},
\texttt{Right\ LT}, \texttt{Right\ EQ}, and \texttt{Right\ GT}. Because
both \texttt{Four} and \texttt{FourSum} possess exactly 4 inhabitants,
they are completely isomorphic!

\paragraph{3. Scaling to 5 Inhabitants}\label{scaling-to-5-inhabitants}

How would you represent a type with exactly 5 inhabitants? Because
\(5 = 2 + 3\), we could use \texttt{Either\ Bool\ Ordering}. Because
\(5 = 1 + 4\), we could also use \texttt{Either\ ()\ (Bool,\ Bool)}.

The specific structures change, but the cardinality remains absolute.
This gives you the superpower of swapping data structures for
mathematical equivalents depending on what is most ergonomic for your
specific problem!

\subsubsection{Section 1.5: Infinite Inhabitants (Countable and
Uncountable)}\label{section-1.5-infinite-inhabitants-countable-and-uncountable}
...
\emph{Categorical Connection:} In abstract algebra, a list of elements
from a set \(X\) forms the \textbf{Free Monoid} over \(X\). Therefore,
the type \texttt{{[}(){]}} represents the Free Monoid over the terminal
object (the singleton set). Because you can endlessly append \texttt{()}
to the sequence, the Free Monoid of a single generator is naturally
isomorphic to the natural numbers \(\mathbb{N}\) and is countably
infinite!
...
\subparagraph{C. Higher Infinities}\label{c.-higher-infinities}

Does the hierarchy of infinity stop there? Not at all! What you are
referring to are \textbf{cardinalities} (which measure the ``size'' of
sets, while ordinals measure ``well-ordered types'').

Thanks to \textbf{Cantor's Theorem}, we know that the power set of any
set always has a \emph{strictly greater cardinality} than the set
itself. In Haskell, calculating the power set (the set of all subsets)
of a type \texttt{a} is mathematically equivalent to forming the
predicate function \texttt{a\ -\textgreater{}\ Bool}.

If \texttt{Integer\ -\textgreater{}\ Bool} has the uncountably infinite
cardinality of the continuum (\(2^{\aleph_0}\)), what happens if we take
the power set of \emph{that}?

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{{-}{-} A cardinality strictly larger than the continuum! (2\^{}(2\^{}$\textbackslash{}aleph\_0$))}
\KeywordTok{type} \DataTypeTok{HyperPredicate} \OtherTok{=}\NormalTok{ (}\DataTypeTok{Integer} \OtherTok{{-}\textgreater{}} \DataTypeTok{Bool}\NormalTok{) }\OtherTok{{-}\textgreater{}} \DataTypeTok{Bool}

\CommentTok{{-}{-} And we can keep going infinitely!}
\KeywordTok{type} \DataTypeTok{MegaPredicate} \OtherTok{=}\NormalTok{ ((}\DataTypeTok{Integer} \OtherTok{{-}\textgreater{}} \DataTypeTok{Bool}\NormalTok{) }\OtherTok{{-}\textgreater{}} \DataTypeTok{Bool}\NormalTok{) }\OtherTok{{-}\textgreater{}} \DataTypeTok{Bool}
\end{Highlighting}
\end{Shaded}

By continually nesting our functions on the left-hand side, we can
theoretically represent an infinite tower of strictly increasing
infinities (known in set theory as the sequence of Beth numbers
\(\beth_0, \beth_1, \beth_2\dots\)) directly inside Haskell's type
system!

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Annex: The Category Hask}\label{annex-the-category-hask}

\subsubsection{Hask: The Category of Haskell
Types}\label{hask-the-category-of-haskell-types}

Throughout this chapter, we frequently reference \textbf{Hask}, the
category of Haskell types. In Category Theory, a category consists of
three things: 1. \textbf{Objects}: In \texttt{Hask}, the objects are
entirely defined by Haskell types (e.g., \texttt{Int}, \texttt{Bool},
\texttt{String}, \texttt{Void}, \texttt{()}, as well as composite types
like \texttt{{[}Int{]}}, \texttt{Maybe\ Double}, or
\texttt{Const\ String\ Bool}). 2. \textbf{Morphisms (Arrows)}: The
morphisms between objects are standard Haskell functions (e.g.,
\texttt{f\ ::\ Maybe\ Double\ -\textgreater{}\ Int} is a morphism from
the object \texttt{Maybe\ Double} to the object \texttt{Int}). Because
function types are themselves objects in \texttt{Hask}, a curried
function like
\texttt{g\ ::\ String\ -\textgreater{}\ Int\ -\textgreater{}\ Int} is
perfectly valid---it is a morphism from the object \texttt{String} to
the object \texttt{Int\ -\textgreater{}\ Int}. 3. \textbf{Composition}:
Morphisms must satisfy composition (in Haskell, using the \texttt{(.)}
operator). If you have a morphism \texttt{f\ ::\ a\ -\textgreater{}\ b}
and a morphism \texttt{g\ ::\ b\ -\textgreater{}\ c}, their composition
\texttt{g\ .\ f} is a valid morphism from
\texttt{a\ -\textgreater{}\ c}. For example, composing
\texttt{show\ ::\ Int\ -\textgreater{}\ String} and
\texttt{length\ ::\ String\ -\textgreater{}\ Int} forms a new morphism
\texttt{(length\ .\ show)\ ::\ Int\ -\textgreater{}\ Int}.

Furthermore, a valid category must obey two strict mathematical laws: *
\textbf{Identity}: Every object must have an identity morphism
(\texttt{id\ ::\ a\ -\textgreater{}\ a}) that behaves as a no-op:
\texttt{f\ .\ id\ ==\ id\ .\ f\ ==\ f}. * \textbf{Associativity}:
Function composition must be identically associative:
\texttt{(f\ .\ g)\ .\ h\ ==\ f\ .\ (g\ .\ h)}.

Haskell was explicitly designed so its abstractions securely mirror
these categorical concepts. However, there is a catch. As famously
pointed out by Andrej Bauer in his seminal post \emph{``Hask is not a
category''}, is \texttt{Hask} strictly a valid category from a rigorous
mathematical standpoint? The technical answer is \textbf{No}, and it's
because of a phenomenon called ``Bottom''.

\emph{(Note: Despite this technicality, the paper ``Fast and Loose
Reasoning is Morally Correct'' (Danielsson et al., 2006) formally
justifies why we can safely abstract away Bottom when reasoning about
programs. Therefore, throughout this entire document, we will follow
standard functional programming convention and \textbf{assume that Hask
is a valid category}.)}
...
\paragraph{Exercise: Compiling the
Impossible}\label{exercise-compiling-the-impossible}

Haskell's \texttt{undefined} is a built-in value that evaluates to
bottom (\texttt{\_\textbar{}\_}). Because it can crash the program, the
compiler assigns it the universal type \texttt{a}---meaning it can
substitute for literally \emph{anything}. This makes it incredibly
useful as a temporary placeholder when developing programs.

Can you write a compiling Haskell snippet that defines a function
promising to convert a \texttt{String} into the impossible \texttt{Void}
type, using \texttt{undefined} to satisfy the type checker?
...
\CommentTok{{-}{-} Mathematically impossible to produce a Void, but the compiler accepts it!}
\OtherTok{cheatTheSystem ::} \DataTypeTok{String} \OtherTok{{-}\textgreater{}} \DataTypeTok{Void}
\NormalTok{cheatTheSystem \_ }\OtherTok{=} \FunctionTok{undefined}
\end{Highlighting}
\end{Shaded}

Because \texttt{undefined} evaluates to bottom, the compiler accepts it
as a valid return value for \emph{every} type, including \texttt{Void}.
As long as you don't actually \emph{call} \texttt{cheatTheSystem} with a
\texttt{String} at runtime (which would immediately crash the program),
the file will compile perfectly. This exact principle is how Haskellers
use ``typed holes'' and \texttt{undefined} to incrementally build and
type-check large systems before all the pieces are actually finished!
...
Boolean type: 1. \textbf{Contradiction} (\(\bot\)): Always returns
\texttt{False} (ignores both inputs). 2. \textbf{NOR} (\(\downarrow\)):
Returns \texttt{True} only if both are \texttt{False}. 3.
\textbf{Converse Nonimplication} (\(\nleftarrow\)): Returns
\texttt{True} only if \(B\) is True and \(A\) is False. 4.
\textbf{Negation A} (\(\neg\)A): Always returns \texttt{Not\ A} (ignores
the second argument). 5. \textbf{Material Nonimplication}
(\(\nrightarrow\)): Returns \texttt{True} only if \(A\) is True and
\(B\) is False. 6. \textbf{Negation B} (\(\neg\)B): Always returns
\texttt{Not\ B} (ignores the first argument). 7. \textbf{XOR}
(\(\oplus\)): Returns \texttt{True} if inputs are different. 8.
\textbf{NAND} (\(\uparrow\)): Returns \texttt{False} only if both are
\texttt{True}. 9. \textbf{AND} (\(\wedge\)): Returns \texttt{True} only
if both are \texttt{True}. 10. \textbf{Equivalence}
(\(\leftrightarrow\)): Returns \texttt{True} if inputs are the same. 11.
\textbf{Projection B} (B): Always returns \(B\) (ignores the first
argument). 12. \textbf{Material Implication} (\(\to\)): Returns
\texttt{False} only if \(A\) is True and \(B\) is False. 13.
\textbf{Projection A} (A): Always returns \(A\) (ignores the second
argument). 14. \textbf{Converse Implication} (\(\leftarrow\)): Returns
\texttt{False} only if \(B\) is True and \(A\) is False. 15. \textbf{OR}
(\(\vee\)): Returns \texttt{True} if at least one is \texttt{True}. 16.
\textbf{Tautology} (\(\top\)): Always returns \texttt{True} (ignores
both inputs).
...
NAND, Negation B, Material Nonimplication (\(\nrightarrow\)), and
Converse Implication (\(\leftarrow\)). Discarding those 9 leaves us with
exactly 7 operations possessing a valid left identity.
...
the right. * \textbf{Material Implication} (\(\to\)):
\texttt{T\ \$\textbackslash{}to\$\ x\ =\ x} (Left Identity is
\texttt{T}), but \texttt{x\ \$\textbackslash{}to\$\ T\ =\ True} (Fails
Right Identity). * \textbf{Converse Nonimplication} (\(\nleftarrow\)):
\texttt{F\ \$\textbackslash{}nleftarrow\$\ x\ =\ x} (Left Identity is
\texttt{F}), but \texttt{x\ \$\textbackslash{}nleftarrow\$\ F\ =\ False}
(Fails Right Identity).
...
\item
  ...
  Coproducts} and Algebraic signatures. \# Part 6: The Deep Math
\end{enumerate}
...
refer to \href{09_bibliography.md}{Part 9: Bibliography}. \# Part 9:
Bibliography
\end{quote}
...
\item
  \textbf{Taylor, C. (2013).}
  \emph{\href{https://gist.github.com/gregberns/5e9da0c95a9a8d2b6338afe69310b945}{The
