...
- **src/Lambda/**
    ...
    - `Lens.hs`: Advanced data access patterns using `Control.Lens` (Prisms, makeFields, Plated universe).
    - `ListAverage.hs`: Exploring `Foldable` and `Traversable` by calculating list averages across different categorical abstractions (Case, Monad, Applicative, State).
...
- **test/Lambda/**
    ...
    - `LensTest.hs`: Tests for the custom Lens implementation using Control.Lens.
    - `ListAverageTest.hs`: Unit and property tests for List Average computations using different algorithms.
...
- **`calc2`**: Demonstrates monadic composition using `myDiv` and `mySum`. It calculates the sum of `div a b` and `div b a` safely.
